...
function VersionCheck(requires_version)
  local curver = {}
  curver[1], curver[2], curver[3], curver[4] =
...
function XMLDecode(s)
  ...
  s = string.gsub(s, "%<%!%[CDATA%[(.-)%]%]%>", function(a)
    return a
  end)
...
function HideProxyInAllRooms(idBinding)
  ...
  if idBinding == 0 then
    return
  end -- silently fail if no binding passed in.
...
function Deserialize(b64)
  if type(b64) == "string" then
    local b64DecodeSuccess, j = pcall(C4.Base64Decode, C4, b64)
    if b64DecodeSuccess and j then
      local jsonDecodeSuccess, d = pcall(JSON.decode, JSON, j)
...
function GetRandomString(length, mask)
  assert(type(length) == "number" and length > 0, "argument 'length' must be an integer greater than 0")
  assert(
    mask == nil or (type(mask) == "string" and not IsEmpty(mask)),
    "argument 'mask' must be a non-empty string when supplied"
  )
  ...
  local s = {}
  while #s < length do
    local max = string.len(mask)
    local random = math.random(1, max)
    local char = string.sub(mask, random, random)
    table.insert(s, char)
  end
  s = table.concat(s)
  return s
